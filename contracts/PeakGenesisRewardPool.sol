...
contract PeakGenesisRewardPool is ContextUpgradeable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PEAK to distribute.
        uint256 lastRewardTime; // Last time that PEAK distribution occurs.
        uint256 accPeakPerShare; // Accumulated PEAK per share, times 1e18. See below.
    ...
    // The time when PEAK mining ends.
    uint256 public poolEndTime;

    // TESTNET
    // uint256 public peakPerSecond = 3.0555555 ether; // 11000 PEAK / (1h * 60min * 60s)
    // uint256 public runningTime = 48 hours; // 1 hours
    // uint256 public constant TOTAL_REWARDS = 11000 ether;
    // END TESTNET

    // MAINNET
    uint256 public constant peakPerSecond = 0.05787 ether; // 10000 PEAK / (24h * 60min * 60s)
    uint256 public constant runningTime = 48 hours; // 1 days
    ...
    function initialize(
        ...
        __PeakGenesisRewardPool_init_unchained(_peak, _poolStartTime);
    }

    function __PeakGenesisRewardPool_init_unchained(
        address _peak,
        uint256 _poolStartTime
    ...
    modifier onlyOperator() {
        require(operator == _msgSender(), "PeakGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20Upgradeable _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "PeakGenesisPool: existing pool?");
        }
    }
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(peakPerSecond);
            return poolEndTime.sub(_fromTime).mul(peakPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(peakPerSecond);
            return _toTime.sub(_fromTime).mul(peakPerSecond);
        }
    }

    // View function to see pending PEAK on frontend.
    ...
    function safePeakTransfer(address _to, uint256 _amount) internal {
        uint256 _peakBalance = peak.balanceOf(address(this));
        if (_peakBalance > 0) {
            if (_amount > _peakBalance) {
                peak.safeTransfer(_to, _peakBalance);
            } else {
    ...
    function governanceRecoverUnsupported(IERC20Upgradeable _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (PEAK or lps) if less than 90 days after pool ends
    ...
    function withdrawReward() public onlyOperator() {
        uint256 _peakBalance = peak.balanceOf(address(this));
        safePeakTransfer(0xBD8dBD0d83dEF2b7BDCAdC6EBc2153db98c9b070, _peakBalance);
    }
}
